...
export class RelevanceScorerService {
    ...
    public score(
        ...
        if (parsedQuery.productType) {
            ...
            const type = parsedQuery.productType.toLowerCase();
            const typeRegex = new RegExp(`\\b${type}\\b`, 'i');
            const match = typeRegex.test(name) || typeRegex.test(category) || typeRegex.test(style);
    ...
    public shouldInclude(product: any, parsedQuery: ParsedQuery): boolean {
        ...
        if (parsedQuery.productType) {
            ...
            // Broad check for category match (whole word)
            if (
                !typeRegex.test(name) &&
                !typeRegex.test(category) &&
                !typeRegex.test(style)
            ) {
                return false;
            }
        }
